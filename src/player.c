...
static void adjust_level(struct player *p, bool verbose)
{
	...
	while ((p->lev < PY_MAX_LEVEL) &&
		...
		else if (p->lev > p->max_lev) {
			if (randint0(9) < ABS(p->p_luck) * 2) p->p_luck++;
		}

		/* Save the highest level (after level-gain curing which happens only the first time you reach a level) */
...
void player_flags_timed(struct player *p, bitflag f[OF_SIZE])
{
	...
	if (p->timed[TMD_OPP_CONF] || player->timed[TMD_CLEAR_MIND]) {
