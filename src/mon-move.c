 ...
 * ------------------------------------------------------------------------ */
/**
 * From Will Asher in DJA: Hey, that's me. :)
							   ...
							   struct loc grid)
{
	...
	/* monsters ignore open pits for now (RBTODO) */
...
static void get_move_find_range(struct monster *mon)
{
	...
	if (!(mon->min_range < flee_range)) {
		mon->min_range = flee_range;
	} else if ((mon->cdis < z_info->turn_range) && (!rf_has(mon->race->flags, RF_KEEP_DIST))) {
...
static bool get_move(struct chunk *c, struct monster *mon, int *dir, bool *good)
{
	...
	if ((monster_can_see_player(c, mon)) && (mon->cdis < mon->min_range) && !mon->m_timed[MON_TMD_CONF]) {
		int badluck = 0;
		...
		if ((mon->cdis < mon->min_range - 1) && ((randint0(100) < 50 + (mon->min_range - mon->cdis) * 5) || 
			(monster_taking_terrain_damage(mon)))) {
			/* Move away from the player (copied from "just leg it away from the player") */
			grid = loc_diff(loc(0, 0), grid);
			/* Check if the monster can move away from the PC before stopping (not sure if this is needed) */
			if (!square_monster(c, grid) && square_ispassable(c, grid))
				return true;
		}
		/* Occationally get another chance for a ranged attack */
			...
			if (make_ranged_attack(mon)) return false;
		}
		/* Monster might not move */
		if ((mon->cdis > 1) && (randint0(100) < 50)) return false;
	}
...
static void monster_turn(struct chunk *c, struct monster *mon)
{
	...
	if (did_something && monster_is_camouflaged(mon) && 
		monster_is_visible(mon) && monster_is_in_view(mon))
		become_aware(mon);
}
